...
Topology:
    - exchange 'admin_events' (topic, durable) - выделенный для событий Admin Service
    - routing keys: 'studio.created', 'studio.updated', 'studio.deactivated',
...
ADMIN_EXCHANGE_NAME = "admin_events"
...
class OutboxPublisherWorker:
    ...
    def __init__(
        ...
        self._publisher = EventPublisher(
            ...
            exchange_name=ADMIN_EXCHANGE_NAME,
        )
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Admin OutboxPublisherWorker started: exchange=%s poll_interval=%ss batch_size=%s",
            ADMIN_EXCHANGE_NAME, self._poll_interval, self._batch_size,
        )
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.warning("Admin OutboxPublisherWorker stop timeout, cancelling task")
            self._task.cancel()
            try:
                await self._task
        ...
        logger.info("Admin OutboxPublisherWorker stopped")
    ...
    async def _run_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                logger.exception("Admin OutboxPublisherWorker iteration failed: %s", exc)
                try:
                    await asyncio.wait_for(
                        self._stop_event.wait(),
                        timeout=self._poll_interval,
    ...
    async def _publish_one(self, event: EventOutbox) -> None:
        ...
        try:
            ...
            logger.debug(
                "Admin outbox event published: id=%s event_type=%s aggregate=%s:%s",
            ...
            logger.error(
                "Admin outbox publish failed: id=%s event_type=%s attempts=%s error=%s",
                event.id, event.event_type, event.published_attempts, exc,
            )
            
            if event.published_attempts >= self._max_attempts:
                logger.critical(
                    "Admin outbox event exceeded max_attempts and will be skipped: "
