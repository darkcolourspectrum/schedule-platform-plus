...
Topology:
    - exchange 'auth_events' (topic, durable) - выделенный для событий Auth Service
    - routing keys: 'user.created', 'user.updated', 'user.deactivated', 'role.changed'
"""
...
AUTH_EXCHANGE_NAME = "auth_events"
...
class OutboxPublisherWorker:
    ...
    def __init__(
        ...
        self._publisher = EventPublisher(
            ...
            exchange_name=AUTH_EXCHANGE_NAME,
        )
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "OutboxPublisherWorker started: exchange=%s poll_interval=%ss batch_size=%s",
            AUTH_EXCHANGE_NAME, self._poll_interval, self._batch_size,
        )
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.warning("OutboxPublisherWorker stop timeout, cancelling task")
            self._task.cancel()
            try:
                await self._task
    ...
    async def _run_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Защита от падения цикла при любой неожиданной ошибке.
                logger.exception("OutboxPublisherWorker iteration failed: %s", exc)
                try:
                    await asyncio.wait_for(
                        self._stop_event.wait(),
                        timeout=self._poll_interval,
    ...
    async def _process_batch(self) -> int:
        ...
        async with self._session_factory() as session:
            # Загружаем batch unpublished событий с блокировкой строк -
            # чтобы при горизонтальном масштабировании несколько воркеров
            # не подбирали одни и те же события (FOR UPDATE SKIP LOCKED).
            query = (
                select(EventOutbox)
            ...
            # Один commit на batch - блокировки строк снимаются здесь
    ...
    async def _publish_one(self, event: EventOutbox) -> None:
        ...
        try:
            ...
            logger.debug(
                "Outbox event published: id=%s event_type=%s aggregate=%s:%s",
            ...
            logger.error(
                "Outbox publish failed: id=%s event_type=%s attempts=%s error=%s",
                event.id, event.event_type, event.published_attempts, exc,
            )
            
            if event.published_attempts >= self._max_attempts:
                logger.critical(
                    "Outbox event exceeded max_attempts and will be skipped: "
