...
Topology:
    - exchange 'crm_events' (topic, durable) - выделенный для событий CRM Service;
    - routing keys: 'lead.created', 'lead.status_changed' (в будущем
      'lead.converted').
"""
...
class OutboxPublisherWorker:
    ...
    def __init__(
        ...
        self._publisher = EventPublisher(
            ...
            exchange_name=CRM_EXCHANGE_NAME,
        )
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(
            self._run_loop(), name="crm-outbox-publisher"
        )
        logger.info(
            "CRM OutboxPublisherWorker started: exchange=%s "
            "poll_interval=%ss batch_size=%s",
            CRM_EXCHANGE_NAME, self._poll_interval, self._batch_size,
        )

    async def stop(self) -> None:
        """Graceful shutdown: остановить цикл, дождаться, закрыть RabbitMQ."""
        ...
        try:
            ...
            logger.warning(
                "CRM OutboxPublisherWorker stop timeout, cancelling task"
            )
            self._task.cancel()
            try:
                await self._task
        ...
        logger.info("CRM OutboxPublisherWorker stopped")
    ...
    async def _run_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Обработали полный batch - возможно, есть ещё события,
                ...
                logger.exception(
                    "CRM OutboxPublisherWorker iteration failed: %s", exc
                )
                try:
                    await asyncio.wait_for(
                        self._stop_event.wait(),
    ...
    async def _publish_one(self, event: EventOutbox) -> None:
        ...
        try:
            ...
            logger.debug(
                "CRM outbox event published: id=%s event_type=%s aggregate=%s:%s",
            ...
            logger.error(
                "CRM outbox publish failed: id=%s event_type=%s "
                "attempts=%s error=%s",
            ...
            if event.published_attempts >= self._max_attempts:
                logger.critical(
                    "CRM outbox event exceeded max_attempts and will be "
                    "skipped: id=%s event_type=%s aggregate=%s:%s",
                    event.id, event.event_type,
                    event.aggregate_type, event.aggregate_id,
                )
