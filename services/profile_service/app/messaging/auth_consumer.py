"""
RabbitMQ consumer для событий Auth Service в Profile Service.

Подписывается на topic exchange 'auth_events' через очередь
'profile.auth_events' с биндингом на 'user.*' и 'role.*'.

DLX 'auth_events.dlx' переиспользуется (создан Schedule).
DLQ Profile: profile.auth_events.dlq.
"""
...
QUEUE_NAME = "profile.auth_events"

DLX_NAME = "auth_events.dlx"
DLQ_NAME = "profile.auth_events.dlq"
